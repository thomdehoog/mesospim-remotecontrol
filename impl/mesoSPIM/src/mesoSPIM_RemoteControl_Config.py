...
MCP_INSTRUCTIONS = (
    ...
    "Call get_manual first: it returns the full command reference, the workflows, the completion "
    "contract (a mutation returns an operation; poll get_progress until its status is 'completed' or "
    "'failed' — never assume completion from elapsed time), and the typed error codes. Read "
    "get_info and get_limits before mutating, and never exceed a limit get_limits reports."
)
