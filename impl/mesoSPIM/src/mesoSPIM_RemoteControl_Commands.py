...
Stage movement is intentionally asynchronous. A move returns an accepted operation immediately,
then a short Qt timer checks mesoSPIM's normal position readback until the requested target is
confirmed. Clients use ``get_progress`` to observe completion. This keeps the Core event loop and
both network transports responsive while the stage is moving.
...
_MANUAL_INTERACTION = {
    ...
    "accepted_or_rejected": "Every mutation is either rejected with a typed error or accepted with "
    "{accepted: true, accepted_command, operation}. Accepted means the "
    "input passed validation and the operation was admitted; it does not "
    "by itself prove that hardware reached the requested state. Read-only "
    "commands return their requested data and do not create an operation.",
    "confirm_completion": "After a mutation is accepted, save operation.id and poll get_progress "
    "over TCP or MCP. Match the same operation id and wait for status "
    "'completed' or 'failed'. For movement, completion means mesoSPIM's "
    "position readback reached the accepted target within tolerance. Never "
    "resend an accepted mutation merely because it is still processing.",
    "kinds": {
        "read": "Returns current data without opening or waiting for the mutation gate.",
        "action": "A short mutation completed by Core before its accepted response is returned.",
        "wait": "An asynchronous mutation. Its accepted response normally reports 'processing'; "
        "poll get_progress for the same operation id until it becomes terminal.",
        "emergency": "A safety command allowed while another mutation is processing: stop, "
        "stop_activity, close_shutters, or time_lapse_stop.",
    },
