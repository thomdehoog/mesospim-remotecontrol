...
operation lifecycle, and returns consistent success or error information. It also provides the
strict JSON parser shared by the wire protocols.
...
_PUBLIC_OP_KEYS = ("id", "command", "status", "target", "observed", "stop_requested", "error")
...
def dispatch(core, cmd, args):
    ...
    # The busy check and the begin are ONE atomic step (see _GATE): if two mutations arrive on
    # different threads, exactly one opens an operation and the other gets BusyError — never both.
    # execute() runs OUTSIDE the lock, so a long WAIT (which returns 'processing' and leaves the Core
    # thread free) rejects a competing mutation promptly with busy. A synchronous ACTION can still
    # make a later call wait before its gate check, but stage moves are WAIT commands and therefore
    # do not block reads or transport replies while the stage is travelling.
    with _GATE:
        active = _active(core)
    ...
    try:
        ...
        with _GATE:
            ...
            operation["error"] = str(error)
        raise
    # A short ACTION completes on return. An asynchronous WAIT completes at its milestone.
    if cmd.milestone is None:
        with _GATE:
            operation["status"] = COMPLETED
    return _reply(cmd.name, result, operation)
