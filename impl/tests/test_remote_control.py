...
from fakes import FakeCore, FakeCfg, defer_recorder
...
def test_stage_move_is_nonblocking_and_wraps_reply():
    ...
    assert reply["operation"]["result"] == {"target": {"x": 100.0}}
    assert ("move_absolute", ({"x_abs": 100.0},), {"wait_until_done": False}) == core.calls[0]


def test_action_is_accepted_before_execution_and_result_is_polled():
    core = FakeCore()

    with defer_recorder() as pending:
        reply = run(core, "set_intensity", {"intensity": 42})

        assert reply == {
            "accepted": True,
            "accepted_command": "set_intensity",
            "operation": {
                "id": "op-000001",
    ...
    assert core.calls[0][0] == "set_intensity"
...
def test_execute_failure_marks_operation_failed():
    ...
    try:
        reply = run(core, "move_absolute", {"targets": {"x": 10}})
    finally:
...
def test_move_relative_uses_current_plus_delta_and_serial_fallback():
    ...
    assert reply["operation"]["result"] == {"target": {"x": 10.0}}
...
def test_mcp_tools_call_success_and_error():
    ...
    bad = servers._mcp_reply(
        Acc(),
        {
            "jsonrpc": "2.0",
            "id": 4,
            "method": "tools/call",
            "params": {"name": "move_absolute", "arguments": {"targets": {"x": 999999}}},
        },
    )
...
def test_mcp_notification_and_unknown_method():
    assert (
        servers._mcp_reply(None, {"jsonrpc": "2.0", "method": "tools/list"}) is None
