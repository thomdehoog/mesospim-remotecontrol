...
def defer_recorder():
    """Record timer callbacks so tests can inspect each asynchronous operation phase."""
    pending = []
...
class FakeState:
    """Match the limited mapping interface exposed by production StateSingleton."""
...
class FakeCore:
    ...
    def __init__(self, **state_values):
        ...
        # _state_snapshot reads the ETL values through get_parameter_dict.
        for etl in (
            "etl_l_delay_%",
            "etl_l_ramp_rising_%",
            "etl_l_ramp_falling_%",
    ...
    # The minimal bound signal records emissions and invokes connected slots.
    class _Sig:
        ...
        def emit(self, *args):
            # Prefix signal emissions so tests can distinguish them from direct Core calls.
