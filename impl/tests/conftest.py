"""Offline test bootstrap: put `impl/` on the path and install a fake PyQt5 so the Qt-free core
(Dispatcher/Commands) and the importable parts of Servers load and run without a real Qt.

The one load-bearing behaviour: QtCore.QTimer.singleShot(0, fn) calls fn() IMMEDIATELY, so a
WAIT command's deferred body runs synchronously in-test; tests drive terminal signals explicitly.
"""
import os
import sys
import types

sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))   # impl/ -> `import mesoSPIM.src...`
...
def _install_fake_pyqt5():
    ...
    # --- minimal QtWidgets (test infra only): just enough to construct RemoteControlGUI (test_gui).
    # Inert for the existing suite (nothing else imports QtWidgets) and never touches src.
