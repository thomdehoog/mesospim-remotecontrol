"""Broad coverage: the ~40 commands the core suite doesn't touch, the read documents, the TCP
auth/dispatch path, faithful WAIT ordering, framing edges, and an adversarial sweep."""
import contextlib
import http.client
import json
import os
...
from mesoSPIM.src.mesoSPIM_RemoteControl_Dispatcher import (
    run, complete, fail, operation_snapshot, error_info, COMMANDS, jsonable, precheck,
    ValidationError, BusyError, UnknownCommand, READ, EMERGENCY)
from mesoSPIM.src.mesoSPIM_RemoteControl_Config import (
    MILESTONE_FINISHED, MILESTONE_TIMELAPSE,
    MCP_SERVER_NAME, MCP_PROTOCOL_VERSION, OK_MARKER, MAX_ACQUISITION_PLANES)
...
# An INDEPENDENT pin of each command's kind: the wire matrix derives its expected envelope from
# COMMANDS[name].kind, and production dispatch branches on the same kind — so a mislabel (a READ
# wrongly made ACTION would silently go through the busy gate and return a wrapped envelope) moves
# expectation and behavior together and hides everywhere. Pin the two safety-relevant sets here,
# against hardcoded expectations, so such a mislabel fails loudly.
_READ_COMMANDS = {
    "hello", "ping", "get_state", "get_position", "get_state_all", "get_config", "get_info",
    "get_limits", "get_capabilities", "get_manual", "get_progress", "self_test",
    "get_acquisition_list", "stat_files", "get_disk_space", "check_motion_limits"}
_EMERGENCY_COMMANDS = {"stop", "stop_activity", "close_shutters", "time_lapse_stop",
...
@pytest.mark.parametrize("camera_parameters", [
    {}, {"x_pixels": 2048}, {"x_pixels": "wide", "y_pixels": 2048}, {"y_pixels": None}])
@pytest.mark.parametrize("name,args", [
    ("get_config", {}), ("acquire_start", {"acquisition": _acquisition()})])
...
def test_etl_happy_path_returns_readback():
    ...
    readback = {k: v for k, v in reply.items() if k not in ("accepted", "accepted_command", "operation")}
    assert "etl_l_amplitude" in readback and "etl_r_offset" in readback
    assert ("emit:sig_state_request_and_wait_until_done", ({"set_etls_according_to_laser": "488 nm"},), {}) in \
...
def test_set_acquisition_list_publishes_the_same_list_to_the_gui_bridge():
    ...
    run(core, "set_acquisition_list", {
        "acquisitions": [_acquisition()], "selected_row": 0})
...
def test_wait_stays_processing_until_body_and_signal():
    ...
    with defer_recorder() as pending:
        ...
        pending[0]()                                     # fire the deferred Core call
...
def test_stage_move_returns_processing_then_completes_from_readback():
    ...
    with defer_recorder() as pending:
        ...
        assert reply["operation"] == {
            "id": "op-000001", "command": "move_absolute", "status": "processing",
            "target": {"x": 100.0},
        }
        assert core.calls == []
        ...
        assert core.calls == [
            ("move_absolute", ({"x_abs": 100.0},), {"wait_until_done": False})
        ]
        assert operation_snapshot(core)["status"] == "processing"
    ...
    assert operation_snapshot(core) == {
        "id": "op-000001", "command": "move_absolute", "status": "completed",
        "target": {"x": 100.0}, "observed": {"x": 100.0},
    }
...
def test_tcp_validation_error_carries_code():
    ...
    assert "x=999999.0 is outside the allowed range [-25000.0, 25000.0]" in reply   # the valid range is reported back
...
def test_mcp_initialize_exact():
    init = servers._mcp_reply(
        _Acc(FakeCore()), {"jsonrpc": "2.0", "id": 1, "method": "initialize"})
...
# ----- busy gate, end-to-end: a real WAIT op holds the gate against the next mutation -----
...
def test_busy_gate_blocks_second_mutation():
    ...
    assert ("move_absolute", ({"x": 1},), {"wait_until_done": False}) not in \
        [(c[0], c[1], c[2]) for c in core.calls]        # the blocked move never reached the Core


# ----- the transport-thread fail-fast pre-check (precheck): only the unknown-name shortcut -----
# precheck() runs on the calling thread and fast-fails just the one permanent, hardware-free
# rejection — an unknown command name — so it need not wait behind a Core thread that is mid-move.
# Shape, limits and busy are all decided later in run(), with busy LAST (see below).
...
# ----- busy is checked LAST: a fixable mistake surfaces as `validation` even while busy -----

def test_busy_is_checked_after_validation():
    core = FakeCore()
    run(core, "start_live", {})                         # a WAIT holds the gate

    # a call wrong in a FIXABLE way is told so, not masked by busy
    ...
    # a call that is otherwise valid, while busy, is the only one that gets busy
...
# ----- the gate transition is atomic under REAL thread contention (the _GATE lock) -----
# In production every call is marshalled onto the one Core thread, so this can't happen; but the
# safety property ("only one accepted call at a time; the rest are refused busy") must not rely
# ONLY on that. Fire many threads at run() at once and prove the is-busy check and the begin are
# one atomic step: exactly one opens the op, every other is busy. The session read is deliberately
# slowed to WIDEN the check->begin window, so a missing lock would let two threads both see "free"
# and both open one — i.e. this test has teeth (it fails if _GATE is removed), not just under a
# lucky GIL schedule.
...
class _SlowSession(dict):
    def __getitem__(self, key):
        ...
        if key == "operation":
            time.sleep(0.003)                           # read the value, THEN widen the window: two
        return value                                    # unlocked threads both see this stale None
...
# ----- exact call lists: a handler must make ONLY its expected Core call, no stray hardware action -----
...
_GOOD = {"Origin": "http://127.0.0.1", "Authorization": "Bearer secret",
         "Content-Type": "application/json"}

def test_mcp_rejects_wrong_token():
    with _mcp_server() as port:
        status, _ = _post(port, headers={**_GOOD, "Authorization": "Bearer WRONG"},
                          body=b'{"jsonrpc":"2.0","id":1,"method":"initialize"}')
        assert status == 401

def test_mcp_rejects_disallowed_origin():
    with _mcp_server() as port:
        status, _ = _post(port, headers={**_GOOD, "Origin": "http://evil.example"},
                          body=b'{"jsonrpc":"2.0","id":1,"method":"initialize"}')
        assert status == 403
...
def test_mcp_good_request_returns_jsonrpc_result():
    with _mcp_server() as port:
        status, raw = _post(port, headers=_GOOD,
                           body=b'{"jsonrpc":"2.0","id":1,"method":"initialize"}')
        assert status == 200
...
def test_start_mcp_binds_serves_and_stops():
    ...
    try:
        ...
        status, raw = _post(handle.port, headers=_GOOD,
                           body=b'{"jsonrpc":"2.0","id":1,"method":"initialize"}')
        assert status == 200
        assert json.loads(raw)["result"]["serverInfo"]["name"] == MCP_SERVER_NAME
    finally:
...
def test_env_limits_malformed_fails_closed(monkeypatch):
    ...
    var = commands.config.LIMITS_ENV_VAR
    for bad in ("", "   ", "{bad", '["not", "an", "object"]', '{"x": [1]}',
                '{"x": [0, 1, 2]}', '{"x": ["a", "b"]}', '{"x": [true, 1]}',
                '{"nope": [0, 1]}', '{"x": [5, -5]}', '{"x": [0, 1], "x": [0, 2]}'):
...
_PERCENT_15 = ["intensity", "camera_delay_%", "camera_pulse_%", "etl_l_delay_%",
               "etl_l_ramp_rising_%", "etl_l_ramp_falling_%", "etl_r_delay_%",
               "etl_r_ramp_rising_%", "etl_r_ramp_falling_%", "galvo_l_duty_cycle",
               "galvo_r_duty_cycle", "laser_l_delay_%", "laser_l_pulse_%",
               "laser_r_delay_%", "laser_r_pulse_%"]
_SETTER = {"intensity": "set_intensity", "camera_delay_%": "set_camera",
           "camera_pulse_%": "set_camera", "galvo_l_duty_cycle": "set_galvo",
           "galvo_r_duty_cycle": "set_galvo"}


def _setter_call(key, value):
    setter = _SETTER.get(key, "set_etl" if key.startswith("etl_")
                         else "set_laser_timing" if key.startswith("laser_") else None)
...
def test_every_settable_state_key_has_one_closed_validation_policy():
    core = FakeCore()
    classified = (set(commands._cfg_options(core)) | set(commands._PERCENT_KEYS) |
                  set(commands.config.PARAMETER_RANGES) | set(commands._BOOL_STATE_KEYS))
...
def test_all_configured_options_accept_members_and_reject_nonmembers():
    dedicated = {"filter": "set_filter", "zoom": "set_zoom", "laser": "set_laser",
...
def test_acquisition_plane_metadata_round_trips_when_geometry_disagrees():
    ...
    reply = run(core, "set_acquisition_list", {
...
def test_acquisition_plane_count_requires_a_json_integer(bad):
    with pytest.raises(ValidationError, match="positive integer"):
        run(FakeCore(), "acquire_start", {"acquisition": {
            "z_start": 0, "z_end": 0, "z_step": 1, "planes": bad}})
...
def test_clear_cannot_release_a_deferred_action_before_it_starts():
    ...
    assert operation_snapshot(core) == {
        "id": "op-000001", "command": "start_live", "status": "completed",
        "stop_requested": True}
...
def test_preset_within_limits_still_moves():
    ...
    run(core, "load_sample", {})
    assert core.calls == [("move_absolute", ({"y_abs": 45000.0},),
                           {"wait_until_done": False, "use_internal_position": False})]
...
def test_acquire_start_response_keys():
    ...
    keys = {k for k in reply if k not in ("accepted", "accepted_command", "operation")}
    assert keys == {"started", "scheduled", "files", "planes", "pixels"}
    assert reply["planes"] == 2 and reply["pixels"] == [2048, 2048]
