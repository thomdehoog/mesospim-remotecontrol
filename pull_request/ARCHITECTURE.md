...
```text
TCP client ─┐
            ├─> request routing ─> Core thread ─> validation ─> named command ─> mesoSPIM Core
MCP client ─┘
```
...
See [`../impl/INTEGRATION.md`](../impl/INTEGRATION.md) for the exact additions.
...
waits only for Core to accept or reject the call. Long stage movement is not performed inside that
wait.
...
6. Execute or schedule the command.
...
Choose the command kind according to its real behavior:

- `READ` returns current information and never opens the mutation gate.
- `ACTION` is a short mutation that is complete when its execution function returns.
...
Reads return data directly. Mutations return:
...
operation, not a history. An accepted mutation must not be resent merely because it is slow.
...
the GUI thread and waits for the table replacement to finish. This prevents old GUI rows from
overwriting a remote list later.
...
3. The MCP endpoint implements the methods needed here (`initialize`, `tools/list`, `tools/call`),
   not the complete MCP specification.
