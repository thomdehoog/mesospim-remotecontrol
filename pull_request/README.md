...
same 53 named commands through framed TCP or MCP over HTTP.
...
Long work does not keep the initial network request open. A mutation returns an operation ID and
either `processing` or `completed`. Clients poll `get_progress` and match that ID.
...
This keeps TCP and MCP responsive during movement. Clients must not repeat an accepted move while
...
MCP defaults to `http://127.0.0.1:42100/mcp` and uses a Bearer password. It provides `initialize`,
`tools/list`, and `tools/call`. MCP calls are sent to the same command path as TCP calls.
...
acceptance, responsive polling during movement, and readback-based completion. It does not start
mesoSPIM or hardware.
...
- Pull-request suite from source: 209 passed.
- Pull-request suite from the generated patch: 209 passed.
- Ruff: passed.
- Dead-code scan at high confidence: passed.
- Real PyQt widget/shutdown smoke: passed.
- Real PyQt TCP and MCP asynchronous-move smoke: passed.
- Patch applies cleanly to upstream `b3c9638` and the newer `d16546c` integration base.
...
The fresh Windows DemoStage release run verified the asynchronous fix over both transports. MCP
passed its safe movement regression, all 53 commands, and all six adversarial groups. TCP passed all
53 commands and all six adversarial groups. Both runs restored position, settings, acquisition
state, shutters, ETL configuration, and test files. The only remaining operator check is a final
File > Exit teardown on the asynchronous build. See [`../REVIEW_REPORT.md`](../REVIEW_REPORT.md).
