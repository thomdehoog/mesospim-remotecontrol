...
def test_live_demo_all_commands_are_functional_safe_and_restored(request):
    ...
    state_keys = [
        ...
        "etl_r_amplitude", "etl_r_offset", "galvo_l_frequency", "laser_l_delay_%",
    ]
        ...
        for index, name in enumerate(VALID_CASES, 1):
            ...
            try:
                ...
                if name == "save_etl_config":
                    _wait_until(
                        lambda: etl_path.stat().st_mtime_ns != before_save_mtime, "demo ETL save")
    ...
    if connection_lost:
        pytest.fail("remote server connection was lost; mesoSPIM app restart required:\n"
                    + "\n".join(failures))
    ...
    print(f"ALL {TOTAL} COMMANDS VERIFIED; {OPERATIONAL} OPERATIONAL CALLS EXECUTED; "
          f"DEMO STATE RESTORED via {transport}", flush=True)
