"""The recording Core for the wire-integration layer.

Reconciles impl/tests/fakes.py::FakeCore (signals + recording + frame) with the old
``_RecordingCore`` (thread-safe log, ``serial_worker``, ``reset()``). Standardizes the pull_request
wire layer on ``RecordingCore``/``FakeCfg``/``FakeState`` so the concurrent HTTP worker threads and
the test thread share one inspectable, lock-guarded call log.
"""
...
class FakeCfg:
    """TransportConfig: all FIVE axes have a range (theta included), so self_test enforces each."""
    version = "test-1.0"
    ...
    stage_parameters = {
        "x_min": -25000, "x_max": 25000, "y_min": -50000, "y_max": 50000,
        "z_min": -25000, "z_max": 25000, "f_min": 0, "f_max": 98000,
        "theta_min": -999, "theta_max": 999, "stage_type": "DemoStage",
...
class _Sig:
    """A recording bound signal: emit() records the emit under the BARE signal name (so the wire
    matrix matches contracts.EXPECTED_CORE_CALL, which lists sig_stop_movement / sig_save_etl_config
    / sig_state_request_and_wait_until_done by their bare names) and drives connected slots (so the
    real Acceptor completion wiring is exercised). This bare-name recording is the deliberate
    counterpart to impl/tests/fakes.py::_Sig, which records under an "emit:" prefix instead."""
...
class RecordingCore:
    ...
    def _build(self):
        self.state = FakeState(
            state="idle",
            position={"x_pos": 24999.0, "y_pos": 0.0, "z_pos": 0.0,
                      "f_pos": 1000.0, "theta_pos": 0.0},
            position_absolute={"x_pos": 24999.0, "y_pos": 0.0, "z_pos": 0.0,
                               "f_pos": 1000.0, "theta_pos": 0.0},
            shutterconfig="Left", ETL_cfg_file="etl.csv", acq_list=[{}])
    ...
    # --- reads that both record AND return (EXPECTED_CORE_CALL needs the record; the read command
    # needs the value). impl's FakeCore returns WITHOUT recording, which is why impl's _EXPECTED
    # omits these; the pull_request matrix records them. ---
