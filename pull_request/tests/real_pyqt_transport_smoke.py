"""Real-PyQt MCP/TCP smoke for asynchronous stage movement.
...
from mesoSPIM.src.mesoSPIM_RemoteControl_Servers import (
    Acceptor, FrameReader, McpAdapter, TcpAdapter, frame,
)
...
class Core(QtCore.QObject):
    ...
    def __init__(self):
        ...
        position = {
            ...
            "f_pos": 2500.0, "theta_pos": 0.0,
        }
        self.state = {
            "state": "idle", "position": dict(position),
            "position_absolute": dict(position), "current_framenumber": 0,
        }
        self.cfg = SimpleNamespace(stage_parameters={
...
def mcp_call(app, port, name, arguments=None):
    result = {}
...
def tcp_call(app, port, name, arguments=None):
    result = {}
...
def exercise_async_move(app, call):
    ...
    assert accepted["operation"]["target"] == {"x": 100.0}
    ...
    assert progress["operation"] == {
        "id": operation_id, "command": "move_absolute", "status": "completed",
...
def main():
    ...
    for label, adapter_type, caller in (
        ...
        try:
            exercise_async_move(
                app, lambda name, arguments=None: caller(app, port, name, arguments)
            )
            assert core.moves == [{"x_abs": 100.0}]
            print(f"REAL PYQT {label} ASYNC MOVE PASS")
        finally:
            acceptor.close()
            adapter.stop()
            acceptor.stop()

    print("REAL PYQT TRANSPORT POLLING PASS: accepted promptly, polling responsive, target confirmed")
