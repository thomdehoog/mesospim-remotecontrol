...
Each attack class has a named test so failures identify the broken boundary. The exhaustive
command-result matrix lives in ``impl/tests``; this suite proves that the packaged transports reject
the same inputs end to end. Every corpus is bounded and network waits are limited to 0.6 seconds.
