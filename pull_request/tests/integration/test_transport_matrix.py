"""The positive contract matrix: every one of the 53 commands over BOTH lanes returns the correct
envelope BY KIND and reaches (or does not reach) its expected Core call. Deliberately tiny and
greppable; the exhaustive per-command semantics live in impl/tests/.
"""
...
from tests.support.patch_loader import dispatcher
from tests.support.contracts import (
    VALID_CASES, EXPECTED_CORE_CALL, READ_ONLY_WITHOUT_CORE_CALL)

READ, ACTION, WAIT, EMERGENCY = dispatcher.READ, dispatcher.ACTION, dispatcher.WAIT, dispatcher.EMERGENCY
_SPECIAL = {"set_acquisition_list", "acquire_finish"}   # neither table; ACTION -> wrapped/completed
_INLINE_FAKE_TIMER = {
    "move_absolute", "move_relative", "load_sample", "unload_sample", "center_sample",
}
...
def _expected_status(name):
    ...
    if kind == WAIT:
        ...
        return "completed" if name == "preview_acquisition" or name in _INLINE_FAKE_TIMER \
            else "processing"
