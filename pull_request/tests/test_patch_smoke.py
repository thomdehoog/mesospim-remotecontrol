"""Offline smoke check that the shipped patch loads and carries the contract, folds the no-socket
fail-closed checks, and holds the warning-suppression property. Loads via patch_loader (patch OR
source). The exhaustive semantics live in impl/tests/; this proves the packaged wire is intact.
"""
import types
...
def test_allowlist_is_the_documented_53():
    ...
    assert "execute_stage_program" not in dispatcher.COMMANDS  # dropped: opaque stage program can't be bounded
...
def _added_lines_for(patch, path):
    ...
    return [line[1:] for line in section.splitlines()
...
def test_warning_suppression_is_not_part_of_the_patch():
    """The PR must not touch mesoSPIM's warning display: no suppression hook anywhere, and no
    display_warning line added or removed (the stock GUI now lives in ..._Tab.py, outside every
    hunk, so assert the property, not incidental context)."""
    patch = patch_loader.PATCH.read_text(encoding="utf-8")
